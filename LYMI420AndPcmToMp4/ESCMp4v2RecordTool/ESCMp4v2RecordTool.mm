...
@property(nonatomic,assign)NSInteger audioChannel;
...
@property (nonatomic, strong) ESCAACEncoder *aacEncoder;  //!<
...
- (void)startRecordWithFilePath:(NSString *)filePath
    ...
    _mp4Encoder = new CMp4Encoder((char*)cFilePath, (int)width, (int)height, (int)frameRate, (int)10000 / frameRate, (unsigned int)WAVE_FORMAT_AAC, (unsigned int)audioSampleRate, audioChannel, audioBitsPerSample);
    self.audioFormat = audioFormat;
    self.fileName = filePath;
    self.audioChannel = audioChannel;
    self.audioSampleRate = audioSampleRate;
    self.audioBitsPerSample = audioBitsPerSample;
}
...
- (void)addAudioData:(NSData *)audioData isPcm:(BOOL)isPcm{
    if (isPcm) {
        if (!_aacEncoder) {
            self.aacEncoder = [[ESCAACEncoder alloc]init];
            [_aacEncoder setupEncoderWithSampleRate:(int)_audioSampleRate channels:(int)_audioChannel sampleBit:(int)_audioBitsPerSample];
        }
//        if (!_audioEncoder) {
//            self.audioEncoder = [[BBAudioHardEncoder alloc]initWithSampleRate:_audioSampleRate channels:_audioChannel] ;
//        }
//            AudioBufferList bufferList;
//            bufferList.mNumberBuffers = 1;
//            bufferList.mBuffers[0].mNumberChannels = _audioChannel;
//            bufferList.mBuffers[0].mDataByteSize = [audioData length];
//            bufferList.mBuffers[0].mData = (BYTE *)[audioData bytes];
//            [_audioEncoder  encodeWithBufferList:bufferList completianBlock:^(NSData *encodedData, NSError *error) {
//                        NSLog(@"=====================audioData:%@===========================",encodedData);
////
//            }];
//        NSLog(@"=====================audioData:%@===========================",audioData);

        NSData *aacData = [_aacEncoder encodePCMDataWithPCMData:audioData];
        if (aacData) {
            BYTE *frameBuf = (BYTE *)[aacData bytes];
            unsigned int length   = (unsigned int)[aacData length];
            if (_mp4Encoder) {
...
//                    NSLog(@"%@",buff);
                    lastJ = j;
                    [mp4v2Tool addAudioData:buff isPcm:NO];
                }
            }
