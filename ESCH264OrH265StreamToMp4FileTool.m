...
- (instancetype) initWithVideoSize:(CGSize) videoSize filePath:(NSString *)filePath frameRate:(NSInteger)frameRate{
    if (self = [super init]) {
        ...
        self.workQueue = dispatch_queue_create("record  work queue", NULL);
...
//    self.audioWriteInput = [AVAssetWriterInput assetWriterInputWithMediaType:AVMediaTypeAudio outputSettings:NULL sourceFormatHint:cmAudioFormatDescriptionRef];
    ...
    self.audioWriteInput.expectsMediaDataInRealTime = YES;
...
- (void)pushVideoData:(NSData *)videoData{
    ...
    dispatch_async(self.workQueue, ^{
        STRONGSELF
        [strongSelf rtc_pushVideoDataQueue:videoData];
    });
}
- (void) rtc_pushVideoDataQueue:(NSData *)videoData{
    ...
    if (self.videoStarted == NO) {
        ...
        [self.videoWriteInput requestMediaDataWhenReadyOnQueue:self.recordQueue usingBlock:^{
            STRONGSELF
            //取出数据填充
            [strongSelf pushCacheVideoDataWithWate:NO];
        }];
    }else{
        [self pushCacheVideoDataWithWate:NO];
    }
}
- (void)pushCacheVideoDataWithWate:(BOOL)isNot {
    ...
    while (1) {
        ...
        if (_videoFrameIndex >= 64 && isNot) {
            return;
        }
        if (_assetWriter.status == AVAssetWriterStatusUnknown) {
            DLog(@"pushCacheVideoData() status not ready");
            return;
        }
        if ([_videoWriteInput isReadyForMoreMediaData]) {
            ...
            CMSampleBufferRef videoSample = [self videoSampleBufferWithData:videoData formatDescriptor:self.videoFormat dataIndex:self.videoFrameIndex];
             NSLog(@"--------------------------------------(video:%lu) pushCacheVideoData----------------",(unsigned long)_videoDataArray.count);
            if (videoSample) {
...
- (void)pushPCMData:(NSData *)pcmData{
    ...
    dispatch_async(self.workQueue, ^{
        STRONGSELF
        [strongSelf rtc_pushPCMDataQueue:pcmData];
    });
}
- (void)rtc_pushPCMDataQueue:(NSData *)pcmData{
...
//                return;
        ...
        if (_assetWriter.status == AVAssetWriterStatusUnknown) {
            DLog(@"pushCachePCMData() status not ready");
            return;
        }
        //    DLog(@"%ld===%@",aacData.length,aacData);
        if ([self.audioWriteInput isReadyForMoreMediaData]) {
            NSLog(@"--------------------------------------(audio:%lu) pushCachePCMData----------------",(unsigned long)_audioDataArray.count);
...
//    self.vidioIsRecord = NO;

    WEAKSELF
    dispatch_async(self.recordQueue, ^{
        STRONGSELF
        strongSelf.endRecordBlock = handler;
        strongSelf.pushDataIsEnd = YES;
...
- (void)endRecorded {
    ...
    [_assetWriter finishWritingWithCompletionHandler:^{
        STRONGSELF
        DLog(@"finishWriting");
        if (strongSelf.endRecordBlock) {
            strongSelf.endRecordBlock();
        }
    }];
}
